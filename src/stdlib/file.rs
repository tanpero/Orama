use crate::runtime::{Environment, RuntimeError, RuntimeResult, Value};
...
pub fn register(env: &Rc<RefCell<Environment>>) {
    // 打开文件
    super::add_native_fn(env, "file_open", vec!["path", "mode"], |args, _| {
        if args.len() < 2 {
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_open 函数的第一个参数必须是字符串路径".to_string(),
                ));
            }
        };
        ...
        let mode = match &args[1] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_open 函数的第二个参数必须是字符串模式".to_string(),
                ));
            }
        };
        ...
        match options.open(path) {
            Ok(file) => {
                let handle_id = FILE_MANAGER.with(|manager| manager.borrow_mut().register(file));
    ...
    // 读取文件内容
    super::add_native_fn(env, "file_read", vec!["file", "size"], |args, _| {
        if args.len() < 1 {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: args.len(),
        ...
        let file_obj = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_read 函数的第一个参数必须是文件对象".to_string(),
                ));
            }
        };
    ...
    // 写入文件
    super::add_native_fn(env, "file_write", vec!["file", "data"], |args, _| {
        if args.len() < 2 {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 2,
                actual: args.len(),
        ...
        let file_obj = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_write 函数的第一个参数必须是文件对象".to_string(),
                ));
            }
        };
        ...
        let data = match &args[1] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_write 函数的第二个参数必须是字符串".to_string(),
                ));
            }
        };
    ...
    // 关闭文件
    super::add_native_fn(env, "file_close", vec!["file"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let file_obj = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_close 函数的参数必须是文件对象".to_string(),
                ));
            }
        };
        ...
        let result = FILE_MANAGER.with(|manager| manager.borrow_mut().close(handle_id));

        Ok(Value::Boolean(result))
    });

    // 文件是否存在
    super::add_native_fn(env, "file_exists", vec!["path"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_exists 函数的参数必须是字符串路径".to_string(),
                ));
            }
        };
    ...
    // 删除文件
    super::add_native_fn(env, "file_delete", vec!["path"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_delete 函数的参数必须是字符串路径".to_string(),
                ));
            }
        };
    ...
    super::add_native_fn(
        ...
        |args, _| {
            ...
            let old_path = match &args[0] {
                ...
                _ => {
                    return Err(RuntimeError::TypeError(
                        "file_rename 函数的第一个参数必须是字符串路径".to_string(),
                    ));
                }
            };
            ...
            let new_path = match &args[1] {
                ...
                _ => {
                    return Err(RuntimeError::TypeError(
                        "file_rename 函数的第二个参数必须是字符串路径".to_string(),
                    ));
                }
            };
    ...
    // 获取文件大小
    super::add_native_fn(env, "file_size", vec!["path"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "file_size 函数的参数必须是字符串路径".to_string(),
                ));
            }
        };
    ...
    // 读取整个文件内容（便捷函数）
    super::add_native_fn(env, "read_file", vec!["path"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "read_file 函数的参数必须是字符串路径".to_string(),
                ));
            }
        };
    ...
    // 写入整个文件内容（便捷函数）
    super::add_native_fn(env, "write_file", vec!["path", "content"], |args, _| {
        if args.len() < 2 {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 2,
                actual: args.len(),
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "write_file 函数的第一个参数必须是字符串路径".to_string(),
                ));
            }
        };
        ...
        let content = match &args[1] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "write_file 函数的第二个参数必须是字符串内容".to_string(),
                ));
            }
        };
    ...
    // 创建目录
    super::add_native_fn(env, "create_dir", vec!["path"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "create_dir 函数的参数必须是字符串路径".to_string(),
                ));
            }
        };
    ...
    // 删除目录
    super::add_native_fn(env, "remove_dir", vec!["path", "recursive"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "remove_dir 函数的第一个参数必须是字符串路径".to_string(),
                ));
            }
        };
        ...
        let recursive = if args.len() > 1 {
            match &args[1] {
                ...
                _ => {
                    return Err(RuntimeError::TypeError(
                        "remove_dir 函数的第二个参数必须是布尔值".to_string(),
                    ));
                }
            }
        } else {
    ...
    // 列出目录内容
    super::add_native_fn(env, "list_dir", vec!["path"], |args, _| {
        if args.is_empty() {
            return Err(RuntimeError::ArgumentMismatch {
                expected: 1,
                actual: 0,
        ...
        let path = match &args[0] {
            ...
            _ => {
                return Err(RuntimeError::TypeError(
                    "list_dir 函数的参数必须是字符串路径".to_string(),
                ));
            }
        };
