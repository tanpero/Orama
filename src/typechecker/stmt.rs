...
impl<'a> StmtTypeChecker<'a> {
    ...
    pub fn infer_stmt(&mut self, stmt: &Stmt) -> TypeResult<()> {
        match stmt {
            ...
            Stmt::VariableDecl(name, type_annotation, initializer) => {
                self.infer_variable_decl(name, type_annotation, initializer)
            }
            Stmt::TypeDecl(name, type_params, type_def) => {
                self.infer_type_decl(name, type_params, type_def)
            }
    ...
    // 推导变量声明
    fn infer_variable_decl(
        &mut self,
        name: &str,
        type_annotation: &Option<crate::ast::TypeAnnotation>,
    ...
    fn infer_type_decl(
        ...
        self.checker
            .env
            .add_type(name.clone().to_string(), ty.clone());
